...
const controllerPath = require.resolve('../../../../controllers/timeline/postSupplementReactions.controller');
const routerPath = require.resolve('../../../../routes/timeline/postSupplementReactions.route');
...
describe('postSupplementReactionsのルーティング', () => {
  test('POST /supplement/reaction は JWTを必須とし、 post_id/supplement_id/type 検証', async () => {
    const res = await request(app).post('/psupreact/supplement/reaction').send({
  ...
  test('POST /supplement/reaction/delete は JWTを必須とし、 post_id/supplement_id/reaction_id 検証', async () => {
    const res = await request(app).post('/psupreact/supplement/reaction/delete').send({
      post_id: '507f1f77bcf86cd799439001',
      supplement_id: '507f1f77bcf86cd799439002',
