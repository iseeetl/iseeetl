...
const controllerPath = require.resolve(
  '../../../../../controllers/timeline/guest/guestPostSupplementReactions.controller'
);
const routerPath = require.resolve('../../../../../routes/timeline/guest/guestPostSupplementReactions.route');
...
const setupWithMocks = () => {
  ...
  jest.doMock(controllerPath, () => ({
    createSupplementReaction: jest.fn((req, res) =>
      res.status(200).json({ ok: true, action: 'createSupplementReaction' })
    ),
    deleteSupplementReaction: jest.fn((req, res) =>
      res.status(200).json({ ok: true, action: 'deleteSupplementReaction' })
    ),
  }));
...
describe('guestPostSupplementReactionsのルーティング', () => {
  test('POST /supplement/reaction は guest_token を必須とし、 guest_id/guest_name/post_id/supplement_id/type 検証', async () => {
    const res = await request(app).post('/gpsr/supplement/reaction').send({
      guest_name: 'Guest',
      post_id: '507f1f77bcf86cd799439001',
      supplement_id: '507f1f77bcf86cd799439002',
      type: 'like',
    });

    expect(res.status).toBe(200);
    expect(res.body.action).toBe('createSupplementReaction');
    ...
    expect(ctrl.createSupplementReaction).toHaveBeenCalled();
  });

  test('POST /supplement/reaction/delete は guest_token を必須とし、 guest_id/guest_name/post_id/supplement_id/reaction_id 検証', async () => {
    const res = await request(app).post('/gpsr/supplement/reaction/delete').send({
      guest_name: 'Guest',
      post_id: '507f1f77bcf86cd799439001',
      supplement_id: '507f1f77bcf86cd799439002',
    ...
    expect(res.status).toBe(200);
    expect(res.body.action).toBe('deleteSupplementReaction');
    ...
    expect(ctrl.deleteSupplementReaction).toHaveBeenCalled();
  });
});
