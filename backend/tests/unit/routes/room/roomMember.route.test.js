...
const controllerPath = require.resolve('../../../../controllers/room/roomMember.controller');
const queryToBodyPath = require.resolve('../../../../routes/_shared/queryToBody');
const routerPath = require.resolve('../../../../routes/room/roomMember.route');
...
const setupWithMocks = () => {
  ...
  jest.doMock(memberPath, () => ({
    ...
    validateRoomMemberInviteToken: jest.fn(() => pass()),
  }));
  ...
  jest.doMock(controllerPath, () => ({
    ...
    contains: jest.fn((req, res) => res.status(200).json({ ok: true, action: 'contains' })),
...
describe('roomMemberのルーティング', () => {
  test('POST / は JWTを必須とし、room_idを検証する', async () => {
    const res = await request(app).post('/roommember/').send({ room_id: '507f1f77bcf86cd799439001' });
  ...
  test('POST /invite は JWTを必須とし、room_id/periodを検証する', async () => {
    const res = await request(app)
      .post('/roommember/invite')
      .send({
  ...
  test('POST /create は JWTを必須とし、room_id/invite_tokenを検証する', async () => {
    const res = await request(app).post('/roommember/create').send({
      room_id: '507f1f77bcf86cd799439001',
      invite_token: '123456789xxx',
    });
    ...
    expect(ensureJwt).toHaveBeenCalled();
    expect(baseValidate.validateMongoId).toHaveBeenCalledWith('room_id');
    expect(memberValidate.validateRoomMemberInviteToken).toHaveBeenCalledWith('invite_token');
    expect(ctrl.create).toHaveBeenCalled();
  });

  test('POST /delete は JWTを必須とし、_id と room_idを検証する', async () => {
    const res = await request(app)
      .post('/roommember/delete')
      .send({ _id: '507f1f77bcf86cd799439099', room_id: '507f1f77bcf86cd799439001' });
  ...
  test('POST /leave は JWTを必須とし、room_idを検証する', async () => {
    const res = await request(app).post('/roommember/leave').send({ room_id: '507f1f77bcf86cd799439001' });
  ...
  test('POST /contains は JWTを必須とし、room_idを検証する', async () => {
    const res = await request(app).post('/roommember/contains').send({ room_id: '507f1f77bcf86cd799439001' });

    expect(res.status).toBe(200);
    expect(res.body.action).toBe('contains');

    expect(ensureJwt).toHaveBeenCalled();
    expect(baseValidate.validateMongoId).toHaveBeenCalledWith('room_id');
    expect(ctrl.contains).toHaveBeenCalled();
  });

  test('POST /management/paginate は body を使い JWT認証と管理者権限を確認し、ページを検証する', async () => {
    const body = { page: { limit: 10, offset: 0 } };
    const res = await request(app)
      .post('/roommember/management/paginate')
      .send(body);
  ...
  test('GET /management/paginate はクエリを body に割り当てて同じミドルウェア列を通る', async () => {
    const res = await request(app).get('/roommember/management/paginate?page=7');
    ...
    expect(sharedMiddleware.finalize).toHaveBeenCalled();
    expect(ctrl.managementPaginate).toHaveBeenCalled();
    expect(ctrl.managementPaginate.mock.calls[0][0].body).toEqual({ page: '7' });
  });

  test.each(['put', 'patch', 'delete'])(
    '%s /management/paginate は 404 となり対象ミドルウェアとコントローラへ到達しない',
    async (method) => {
      const res = await request(app)[method]('/roommember/management/paginate').send({ page: 7 });
  ...
  test('POST /management/delete は JWTを必須とし、_idを検証する', async () => {
    const res = await request(app).post('/roommember/management/delete').send({ _id: '507f1f77bcf86cd799439099' });
