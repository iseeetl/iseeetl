...
const createWrapper = (overrides = {}) =>
  shallowMount(UpdateRoomDisplayConfirm, {
    stubs: baseStubs,
    props: {
      confirmVisible: true,
...
describe('ルームの表示・非表示の確認', () => {
  it('非表示を解除する操作を公開ではなく表示と案内する', () => {
    const wrapper = createWrapper({ props: { roomDisplayHidden: false } });
    const dialog = wrapper.findComponent(UiDialogStub);

    expect(wrapper.text()).to.include('全ルーム表示');
    expect(wrapper.text()).to.include('現在のフロア内に存在する全てのルームを表示します');
    ...
    expect(dialog.props('initialFocus')).to.equal('.update-room-display-cancel');
  });

  it('非表示操作を文言とアイコンで明示する', () => {
    const wrapper = createWrapper({ props: { roomDisplayHidden: true } });

    expect(wrapper.text()).to.include('非表示にする');
    expect(wrapper.findAll('[data-icon="visibility_off"]')).to.have.lengthOf(2);
  });

  it('現在の状態でdoneをemitし、親の処理完了までは閉じない', async () => {
    const wrapper = createWrapper({ props: { roomDisplayHidden: true } });
  ...
  it('送信中は閉じる操作と再送信を禁止し、進行状況を表示する', () => {
    const wrapper = createWrapper({ props: { sending: true, roomDisplayHidden: true } });
    const dialog = wrapper.findComponent(UiDialogStub);
