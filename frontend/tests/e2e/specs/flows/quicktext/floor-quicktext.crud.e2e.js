...
const assertEditorRole = (browser) => {
  browser.execute(
    function () {
      try {
        const persisted = JSON.parse(window.localStorage.getItem('iseeetl_store') || '{}');
        return { role: persisted.user && persisted.user.role ? String(persisted.user.role) : '' };
      } catch (_) {
        return { role: '' };
      }
    },
    [],
    (result) => {
      const role = result && result.value ? result.value.role : '';
      browser.assert.strictEqual(role, 'Editor', 'フロアの単語は固定のフロア編集ユーザで操作します。');
    }
  );
};
...
const confirmDelete = (browser) => {
  waitForConfirmDialog(browser, CONFIRM_DIALOG_ROOT, 'フロアの単語の確認ダイアログ');
  clickSingleVisible(browser, CONFIRM_SELECTOR, 'フロアの単語の変更を確定');
  waitForResourceQuickTextDeletion(browser, 'floor');
};
...
const createGroupAndItem = (browser, groupTitle, itemLabel) => {
  ...
  browser
    .waitForElementVisible('#resource-quicktext-floor-group-value', 10000)
    .setValue('#resource-quicktext-floor-group-value', groupTitle);
  saveResourceQuickTextForm(browser, 'floor', 'group', groupTitle);
  waitForGroupPresence(browser, groupTitle, true);

  clickGroupAction(browser, groupTitle, '.group-top-actions .items-create-btn', '項目を作成');
  browser.waitForElementVisible('#resource-quicktext-floor-item-value', 10000).setValue('#resource-quicktext-floor-item-value', itemLabel);
  saveResourceQuickTextForm(browser, 'floor', 'item', itemLabel);
  waitForItemPresence(browser, groupTitle, itemLabel, true);
};

module.exports = {
  'フロアの単語を作成・編集・削除できる': (browser) => {
    runEditorFlowWithPreparedFloorRoom(browser, 'Floor QuickText CRUD', ({ floorId, roomId, finish }) => {
      openResourceQuickTextDialog(browser, { resource: 'floor', floorId, roomId });
      browser.waitForElementVisible('[data-testid="quicktext-dialog-create-group"]', 10000);
      assertEditorRole(browser);

      const stamp = Date.now();
      const groupTitleA = `E2EFloorGroupA${stamp}`;
      const groupTitleB = `E2EFloorGroupB${stamp}`;
      const itemLabelA = `E2EFloorItemA${stamp}`;
      const itemLabelB = `E2EFloorItemB${stamp}`;
      ...
      browser.waitForElementVisible('#resource-quicktext-floor-group-value', 10000)
        .clearValue('#resource-quicktext-floor-group-value').setValue('#resource-quicktext-floor-group-value', editedGroupTitle);
      saveResourceQuickTextForm(browser, 'floor', 'group', editedGroupTitle);
      ...
      browser.waitForElementVisible('#resource-quicktext-floor-item-value', 10000)
        .clearValue('#resource-quicktext-floor-item-value').setValue('#resource-quicktext-floor-item-value', editedItemLabel);
      saveResourceQuickTextForm(browser, 'floor', 'item', editedItemLabel);
      ...
      openResourceQuickTextDialog(browser, { resource: 'floor', floorId, roomId });
      browser.waitForElementVisible('.group-list', 10000);
      waitForGroupPresence(browser, editedGroupTitle, true);
      waitForItemPresence(browser, editedGroupTitle, editedItemLabel, true);
      waitForGroupPresence(browser, groupTitleA, false);
