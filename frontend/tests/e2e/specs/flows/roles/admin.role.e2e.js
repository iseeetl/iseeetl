...
module.exports = {
  '管理者に管理メニューとフロア・ルームの操作を表示する': (browser) => {
    const mail = process.env.E2E_ADMIN_MAIL || '';
    const password = process.env.E2E_ADMIN_PASSWORD || '';
    const editorMail = process.env.E2E_FLOOR_EDITOR_MAIL || '';
    const editorPassword = process.env.E2E_FLOOR_EDITOR_PASSWORD || '';

    if (!mail || !password || !editorMail || !editorPassword) {
      browser.assert.ok(false, '管理者権限のテストをスキップします。E2E_ADMINまたはE2E_FLOOR_EDITORの認証情報が未設定です。');
      browser.end();
      return;
    }
    ...
    browser.perform(() => {
      ...
      if (!ids.floorId || !ids.roomId) {
        browser.assert.ok(false, '管理者権限のテストに失敗しました。フロアIDまたはルームIDを取得できませんでした。');
        browser.end();
        return;
      }
