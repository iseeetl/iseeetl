<template>
  <div>
    <TimelineEditorDialog
      ...
      :visible="visible"
      title-id="edit_reply_dialog_title"
      :title-text="$t('返信')"
      :cancel-label="$t('キャンセル')"
      initial-focus="#reply_content"
    ...
    >
      <template #mobile-actions>
        <UiButton
          ...
          data-testid="dialog-edit-reply-flow-submit"
        ...
        <UiButton
          ...
          :aria-label="$t('返信')"
          :disabled="sending || recording || transcribing"
          data-testid="dialog-edit-reply-submit"
          @click.stop="submitReply(false)"
        >
          <UiIcon class="reply-submit-icon" name="reply" />
        </UiButton>
      </template>
      ...
      <div class="post" v-if="post">
        ...
        <div class="container">
          ...
          <div class="translate" v-if="needsTranslation(post)">
            {{ getContent(post) }}
          </div>
          <div class="tags" v-if="$store.getters.displayTag">
            <template v-for="(tagId, index) in post.room_tags" :key="index">
              <span v-if="isTagInRoomTags(tagId)"> #{{ getTagNameById(tagId) }} </span>
            </template>
          </div>
        ...
        <fieldset :class="['acc-fieldset', { 'is-collapsed': !isAccordionOpen('reaction') }]">
          <legend class="acc-legend">
            ...
            <button
              ...
              :aria-controls="accIds.reaction"
              :disabled="sending || recording"
              data-testid="dialog-edit-reply-reaction-title-toggle"
          ...
          >
            <legend class="acc-legend">
              ...
              <button
                ...
                :aria-controls="qtPanelId(g._id)"
                :disabled="sending || recording"
                :data-testid="'dialog-edit-reply-quicktext-title-toggle-' + g._id"
        ...
        <fieldset :class="['acc-fieldset', { 'is-collapsed': !isAccordionOpen('tags') }]">
          <legend class="acc-legend">
            ...
            <button
              ...
              :aria-controls="accIds.tags"
              :disabled="sending || recording"
              data-testid="dialog-edit-reply-tags-title-toggle"
          ...
          <div class="acc-panel" :id="accIds.tags" v-show="isAccordionOpen('tags')">
            <tag-selector
              v-model="roomTagsSelected"
              :tags="selectableRoomTags"
              :locale="$i18n.locale"
              :originalTags="post ? post.room_tags : []"
              :previousTags="$store.getters.tagList"
        ...
        <fieldset :class="['acc-fieldset', { 'is-collapsed': !isAccordionOpen('media') }]">
          <legend class="acc-legend">
            ...
            <button
              ...
              :aria-controls="accIds.media"
              :disabled="sending || recording"
              data-testid="dialog-edit-reply-media-title-toggle"
          ...
          <div class="acc-panel" :id="accIds.media" v-show="isAccordionOpen('media')">
            <media-input
              id-prefix="timeline-reply-media"
              :locked="isGuest"
      ...
      <div class="edit-input-field" v-if="$store.getters.userIsLogin">
        <div class="checkbox-group">
          <input
            ...
            :disabled="sending || recording"
          />
          <label for="input_notify_all" class="checkbox-label">
            {{ $t('全員へ通知する') }}
          </label>
        </div>
      </div>
...
export default {
  ...
  name: 'EditReplyDialog',
  setup() {
    const accordionId = useId();
    return {
      v$: useOptionsVuelidate(),
      accIds: {
        reaction: `edit-reply-${accordionId}-reaction`,
        tags: `edit-reply-${accordionId}-tags`,
        media: `edit-reply-${accordionId}-media`,
      },
    };
  },
  components: {
  ...
  computed: {
    ...
    submittableRoomTagIds() {
      return Array.isArray(this.roomTagsSelected) ? this.roomTagsSelected : [];
    },
  ...
  data() {
    return {
      ...
      accordion: { reaction: true, tags: true, media: true },
  ...
  methods: {
    ...
    openedDialog() {
      this.mutationBaseline = this.replyValue ? JSON.parse(JSON.stringify(this.replyValue)) : null;
      document.addEventListener('keydown', this.handleEditorShortcut);

      this.post = this.postValue;
      this.postId = this.postValue._id;

      if (this.replyValue !== null) {
        this.replyId = this.replyValue._id;
        this.content = this.replyValue.content;
        this.roomTagsSelected = this.replyValue.room_tags;
        this.animation = this.replyValue.animation;
        this.imageName = this.replyValue.image_name;
        this.imageThumbnailName = this.replyValue.image_thumbnail_name;
        this.videoName = this.replyValue.video_name;
        this.videoThumbnailName = this.replyValue.video_thumbnail_name;
        this.audioName = this.replyValue.audio_name;

        this.imageCaption = this.replyValue.image_caption;
        this.videoSubtitleOriginalName = this.replyValue.video_subtitle_originalname;
        this.videoSubtitleName = this.replyValue.video_subtitle_name;
        this.audioTitle = this.replyValue.audio_title;
        this.audioDescription = this.replyValue.audio_description;
        ...
        // 新規返信では、以前の投稿や返信のタグを自動で引き継がない。
        this.roomTagsSelected = [];
      }
    ...
    handleEditorShortcut(keyboardEvent) {
      ...
      if (
        ...
        this.submitReply(this.isGuestReactionOnly);
      }
    },
    ...
    getContentTextarea() {
      return this.$refs.replyContentRef || null;
    },
    ...
    isTagInRoomTags(tagId) {
      return TimelineUtil.isTagInRoomTags(this.roomTags, tagId);
    },

    getTagNameById(tagId) {
      const tag = this.roomTags.find((tag) => tag._id === tagId);
    ...
    qtPanelId(groupId) {
      return 'acc-qt-panel-reply-' + String(groupId);
    },
    ...
    async submitReply(isFlow = false) {
      ...
      try {
        ...
        const hasSomething =
          ...
          !!this.audioName;
        if (!hasSomething) {
          this.setSnackbar(this.$t('返信する内容がありません'), 'alert');
          return;
        }
        if (!this.validateMedia()) return;
        const actionType = this.replyId === null ? 'create' : 'update';
        const mediaType = this.imageFile ? 'image' : this.videoFile ? 'video' : this.audioFile ? 'audio' : null;
        const previousTagIds = Array.isArray(this.replyValue?.room_tags) ? [...this.replyValue.room_tags] : [];
        this.animation = isFlow ? 'move-and-erase' : null;
        ...
        if (this.$store.getters.userId) {
          ...
          response = await this.editReply(p(didUpload ? 80 : 0, 100));
        } else {
          response = await this.editGuestReply(p(0, 100));
        }
        successPayload = {
          contentType: 'reply',
          actionType,
    ...
    editReply(onProgress) {
      ...
      let data = {
        ...
        lang: this.replyId ? (this.mutationBaseline?.lang || this.replyValue?.lang || this.$i18n.locale) : this.$i18n.locale,
      ...
      const scope = { room_id: data.room_id, post_id: data.post_id };
      const payload = this.replyId === null ? buildPostCreate(data) : buildPostPatch(data, this.mutationBaseline || this.replyValue || {});
      if (this.notifyAll) payload.notify_all = true;
      const request =
        this.replyId === null
          ? chatApi.createReply({ ...scope, ...payload }, { onUploadProgress: onProgress })
          : chatApi.updateReply({ ...scope, _id: this.replyId, ...payload }, { onUploadProgress: onProgress });

      return request
        .then((res) => {
          return res;
        })
        .catch((e) => this.handleHttpError(e, '返信に失敗しました'));
    },

    editGuestReply(onProgress) {
      let data = {
      ...
      return chatApi
        .createGuestReply(data, { withCredentials: true, onUploadProgress: onProgress })
        .then((res) => {
          return res;
        })
        .catch((e) => {
          return this.handleHttpError(e, '返信に失敗しました');
        });
    },
    ...
    closedDialog(payload) {
      ...
      // 入力や添付がある新規返信では、状態を初期化する前に破棄を確認する。
      const isEmpty = (!this.content || this.content.length === 0) && !hasMedia;
      if (!isEmpty) {
        document.removeEventListener('keydown', this.handleEditorShortcut);
        this.showConfirm();
        return;
    ...
    onLoginConfirmClosed(payload = {}) {
      ...
      if (this.visible) {
        ...
        if (payload.focusRestored === false) {
          this.$nextTick(() => {
            if (this.$refs.replyContentRef && typeof this.$refs.replyContentRef.focus === 'function') {
    ...
    handleHttpError(e, defaultMsgKey = '返信に失敗しました') {
