...
### POST /api/rooms/:room_id/timeline/posts/:post_id/supplements

入力・応答・認可・エラーは[ルーム配下のタイムラインAPI](timeline-post-resources.md)を参照する。

#### データ更新・通知

- スパムワードを置換した本文で投稿の`supplementaries`へ付加情報を追加
- OneSignalが有効で投稿者本人以外が作成した場合は、投稿者のプッシュ設定に応じて通知を試行する。無効な場合は通知だけを省略する
- OneSignalが有効な場合は`PushFilter`に一致する他ユーザへプッシュ通知を試行し、無効な場合は通知だけを省略する
- Socket.IOの`SUPPLEMENT_CREATE`を対象ルームへ送信
- Google Translateが有効な場合は応答処理とは別に本文翻訳を試行し、結果が作成された場合は`SUPPLEMENT_UPDATE`を送信する

### PATCH /api/rooms/:room_id/timeline/posts/:post_id/supplements/:supplement_id
...
- Socket.IOの`SUPPLEMENT_UPDATE`を対象ルームへ送信
- Google Translateが有効で本文または原文言語（`lang`）が変わった場合は、応答処理とは別に翻訳を試行する

### DELETE /api/rooms/:room_id/timeline/posts/:post_id/supplements/:supplement_id
...
- Socket.IOの`SUPPLEMENT_DELETE`を対象ルームへ送信

### POST /api/chat/supplement/reaction
...
- 404: NOT_FOUND（対象付加情報が存在しない）
- 409: CONFLICT（同じユーザが同じ種別のリアクションを追加済み）

#### データ更新・通知

- 付加情報の`reactions`へJWTのユーザIDと種別を追加
- Socket.IOの`SUPPLEMENT_REACTION_CREATE`を対象ルームへ送信

#### 動作上の注意

- 対象付加情報の`delete_flg`を確認しないため、論理削除済み付加情報にもリアクションを追加できる
- 成功応答では論理削除済み付加情報が除外されるため、追加したリアクションは応答上に現れない

### POST /api/chat/supplement/reaction/delete
...
- 401: TOKEN_INVALID / TOKEN_EXPIRED
- 404: NOT_FOUND（対象付加情報またはリアクションが存在しない）

#### データ更新・通知

- 付加情報の`reactions`から対象リアクションを物理削除
- Socket.IOの`REACTION_DELETE`を対象ルームへ送信

#### 動作上の注意

- 対象付加情報の`delete_flg`を確認しないため、論理削除済み付加情報のリアクションも削除できる
- 成功応答では論理削除済み付加情報を除外する
...
- `backend/services/timeline/postSupplements.service.js`
- `backend/services/timeline/postSupplementReactions.service.js`
- `backend/services/timeline/shared/supplementCommon.js`

### テスト

- `backend/tests/integration/routes/timeline.core.int.test.js`
- `backend/tests/unit/services/timeline/postSupplements.service.test.js`
- `backend/tests/unit/services/timeline/postSupplementReactions.service.test.js`
